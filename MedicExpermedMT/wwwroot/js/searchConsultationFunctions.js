...
function buscarPaciente() {
    ...
    if (primerNombre) params.append('primerNombre', primerNombre);
    if (primerApellido) params.append('primerApellido', primerApellido);
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            // Dado que data es un array, asignamos directamente
...
$(document).ready(function () {
    $('#buscarDiagnostico').on('keyup', function () {
        let query = $(this).val();
        ...
        if (query.length > 2) {
            $.ajax({
                ...
                success: function (data) {
                    $('#diagnosticoResults').empty();
                    data.forEach(function (item) {
                        $('#diagnosticoResults').append('<a href="#" class="list-group-item list-group-item-action" onclick="seleccionarNacionalidad(\'' + item.id + '\', \'' + item.nombre + '\')">' + item.nombre + '</a>');
                    });
                },
                error: function () {
                    $('#diagnosticoResults').html('<div class="text-danger">Error al buscar nacionalidades.</div>');
                }
            });
        } else {
            $('#diagnosticoResults').empty();  // Limpiar resultados si el campo está vacío
        }
    });
});
...
function seleccionarNacionalidad(id, nombre) {
    // Establecemos el nombre en el campo visible
    $('#buscarDiagnostico').val(nombre);

    // Establecemos el ID en el campo oculto
    $('#DiagnosticoId').val(id);
...
document.getElementById('consultationForm').addEventListener('submit', async function (event) {
    ...
    const consultaDto = {
        ...
        AntecedentesFamiliares: {
            ...
            ObserCardiopatia: document.getElementById('consulta-observacion-cardiopatia')?.value || null,
            ParentescoCardiopatia: document.getElementById('tipoDocumentoSelectCardiopatia')?.value ? parseInt(document.getElementById('tipoDocumentoSelectCardiopatia')?.value) : null,
            ...
            ParentescoDiabetes: document.getElementById('tipoDocumentoSelectDiabetes')?.value ? parseInt(document.getElementById('tipoDocumentoSelectDiabetes')?.value) : null,
...
document.addEventListener("DOMContentLoaded", function () {
    const checkboxes = document.querySelectorAll('.consulta-antecedente-checked');

    checkboxes.forEach(checkbox => {
        checkbox.addEventListener('change', function () {
            const observationId = `consulta-observacion-${this.id.split('-')[4]}`;
            const observationDiv = document.getElementById(observationId);

            console.log(`Checkbox: ${this.id}, Observation ID: ${observationId}`); // Para verificar el ID generado

            if (observationDiv) { // Asegúrate de que el elemento existe
                if (this.checked) {
                    observationDiv.style.display = 'block';
                    const input = observationDiv.querySelector('input[type="text"]');
                    input.disabled = false;
                } else {
                    observationDiv.style.display = 'none';
                    const input = observationDiv.querySelector('input[type="text"]');
                    input.disabled = true;
                    input.value = ''; // Limpiar el campo de observación al desmarcar
                }
            } else {
                console.error(`No se encontró el elemento con ID: ${observationId}`); // Para ver si hay un error en el ID
            }
        });
    });
});
